...
import { useCartUI } from "../../../features/cart/context/CartContext";
...
const Store = () => {
  ...
  const { setCartOpen, addToCart, cartItemCount } = useCartUI();
  const { toggleWishlist, isInWishlist } = useWishlistUI();
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (selectedCategory !== "all") {
      if (selectedCategory === "__new_arrivals") {
        result = result.filter((p) => p.isNew);
      } else if (selectedCategory === "__bestsellers") {
        result = result.filter((p) => p.isBestseller || p.autoIsBestseller);
      } else {
    ...
    if (priceRange.min) {
      result = result.filter(
        (p) => Number(p.startingPrice?.amount) >= Number(priceRange.min),
      );
    }
    if (priceRange.max) {
      result = result.filter(
        (p) => Number(p.startingPrice?.amount) <= Number(priceRange.max),
      );
    }
    ...
    switch (sortBy) {
      ...
      case "price-low":
        result.sort(
          (a, b) =>
            Number(a.startingPrice?.amount) - Number(b.startingPrice?.amount),
        );
        break;
      case "price-high":
        result.sort(
          (a, b) =>
            Number(b.startingPrice?.amount) - Number(a.startingPrice?.amount),
        );
        break;
      case "popular":
        ...
        <div className="max-w-[1440px] mx-auto px-3 sm:px-4 lg:px-10 py-6 sm:py-12">
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-6">
            {[...Array(8)].map((_, i) => (
              <div
                key={i}
                className="rounded-xl sm:rounded-2xl overflow-hidden bg-[#111111] border border-[#1a1a1a]"
              >
          ...
          <div className="max-w-[1440px] mx-auto px-3 sm:px-4 lg:px-10 py-3 sm:py-4">
            <div className="flex flex-col lg:flex-row lg:items-center gap-3 sm:gap-4">
              <div className="flex items-center gap-2 sm:gap-3">
                <h1 className="text-xl sm:text-2xl font-bold text-[#f0f0f0] font-['Playfair_Display'] tracking-wide">
                  Store
                </h1>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="relative p-2 sm:p-2.5 rounded-lg sm:rounded-xl border border-[#1a1a1a] bg-[#111111] text-[#555555] hover:text-[#f0f0f0] hover:border-[#333333] transition-all"
                >
                ...
                <button
                  ...
                  className={`lg:hidden p-2 sm:p-2.5 rounded-lg sm:rounded-xl border transition-all ${
                    showFilters
                      ? "bg-[#d4a76a]/10 border-[#d4a76a]/30 text-[#d4a76a]"
                      : "bg-[#111111] border-[#1a1a1a] text-[#777777]"
                  }`}
                >
              ...
              <div className="flex items-center gap-2 mt-2 sm:mt-3 flex-wrap">
                ...
                {selectedCategory !== "all" && (
                  <span className="inline-flex items-center gap-1 bg-[#d4a76a]/10 text-[#d4a76a] text-xs sm:text-sm px-2.5 sm:px-3 py-1 rounded-full border border-[#d4a76a]/20">
                    {selectedCategory === "__new_arrivals"
                      ? "New Arrivals"
                      : selectedCategory === "__bestsellers"
                        ? "Bestsellers"
                        : selectedCategory}
                    <button
            ...
            >
              <div className="bg-[#050505] rounded-xl sm:rounded-2xl border border-[#1a1a1a] p-4 sm:p-6 sticky top-20 sm:top-24">
                ...
                <div className="mb-4 sm:mb-6">
                  <h4 className="text-xs font-medium text-[#777777] uppercase tracking-wider mb-2 sm:mb-3">
                    Categories
                  </h4>
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <div className="space-y-1">
                    <button
                      onClick={() => setSelectedCategory("__new_arrivals")}
                    ...
                    >
                      <span className="flex items-center gap-2">
                        ...
                        New Arrivals
                        <span className="ml-auto text-[10px] bg-[#111111] px-1.5 py-0.5 rounded">
                          {products.filter((p) => p.isNew).length}
                        </span>
                      </span>
                    </button>
                    <button
                      onClick={() => setSelectedCategory("__bestsellers")}
                      className={`w-full text-left px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm transition-all ${
                        selectedCategory === "__bestsellers"
                          ? "bg-[#d4a76a]/10 text-[#d4a76a] font-medium border border-[#d4a76a]/20"
                          : "text-[#aaaaaa] hover:bg-[#111111] hover:text-[#f0f0f0]"
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <span className="w-1.5 h-1.5 rounded-full bg-[#c4956a]"></span>
                        Bestsellers
                        <span className="ml-auto text-[10px] bg-[#111111] px-1.5 py-0.5 rounded">
                          {
                            products.filter(
                              (p) => p.isBestseller || p.autoIsBestseller,
                            ).length
                          }
                        </span>
                      </span>
            ...
            <div className="flex-1 min-w-0">
              {filteredProducts.length === 0 ? (
                <div className="bg-[#050505] rounded-xl sm:rounded-2xl border border-[#1a1a1a] p-8 sm:p-12 text-center">
                  ...
                  <h3 className="font-['Playfair_Display'] text-lg sm:text-xl text-[#f0f0f0] mb-1 sm:mb-2">
                    No products found
                  </h3>
                  <p className="text-[#777777] mb-3 sm:mb-4 text-xs sm:text-sm">
              ...
              `}
                >
                  {filteredProducts.map((product, index) => (
                    ...
                    >
                      <div
                        className={`relative overflow-hidden bg-[#111111] ${viewMode === "list" ? "sm:w-80 flex-shrink-0" : "aspect-[3/4]"}`}
                      >
                        {product.thumbnail ? (
                          <img
                        ...
                        >
                          <Heart
                            className={`w-3 h-3 sm:w-4 sm:h-4 ${isInWishlist(product._id) ? "fill-current" : ""}`}
                          />
                        </button>
                      ...
                      <div className="p-2.5 sm:p-5 flex flex-col flex-1">
                        ...
                        <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-2 sm:mb-4">
                          ...
                          {product.tags?.length > 2 && (
                            <span className="text-[9px] sm:text-[10px] text-[#555555] px-0.5 sm:px-1">
                              +{product.tags.length - 2}
                            </span>
                          )}
                        </div>
                        ...
                        <div className="flex items-end justify-between pt-2 sm:pt-4 border-t border-[#1a1a1a]">
                          <div>
                            ...
                            {product.sales > 0 && (
                              <p className="text-[10px] sm:text-[11px] text-[#555555] mt-0.5 sm:mt-1">
                                {product.sales} sold
                              </p>
                            )}
                          </div>
